...
class GPTSoVitsProvider {
    ...
    get settingsHtml() {
        return `
            ...
            <input id="url" type="text" value="${this.settings.url}" />  
    ...
    onSettingsChange() {
        ...
        this.settings.url = document.getElementById('url').value;
    ...
    async checkReady() {
        try {
            const testResponse = await fetch(`${this.settings.url}?text=测试&text_language=${this.settings.text_language}`);
